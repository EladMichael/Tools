...
void assign(double s[],double s0[]);
...
void assign(double s[],double s0[])
{
  s[0]=s0[0];       //Initial X position
  s[1]=s0[1];       //Initial Y position
  s[2]=s0[2];     //Initial X velocity
  s[3]=s0[3];     //Initial Y velocity
}
...
while(prec>=pow(10,-7))
{
 while(j==0)
 {
  ...
  if(a==0){cout<<"Got it! With a timestep of "<<dt<<".\n";break;}
